...
export const MyAccount: React.FC<MyAccountProps> = ({ isOpen, onClose, user, onUpdateUser, onLogout }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-slate-950/80 backdrop-blur-md flex items-start justify-center p-4 pt-14 sm:pt-20 overflow-y-auto">
      <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden text-xs shrink-0">
        {/* Header Compacto */}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between p-2.5 bg-slate-950/60 rounded-lg border border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[9px] uppercase font-bold text-slate-500 tracking-wider">Usuario</p>
                  <p className="text-xs font-semibold text-white">{activeUser.name}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[9px] uppercase font-bold text-slate-500 tracking-wider">Rol</p>
            ...
            {/* PIN de Acceso */}
            <div className="p-2.5 bg-slate-950/80 rounded-lg border border-indigo-500/30">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-0.5 px-1.5 text-slate-400 hover:text-white rounded bg-slate-800 transition text-[10px] flex items-center gap-1 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    disabled={!pin}
                    className="p-0.5 px-1.5 text-slate-400 hover:text-indigo-300 rounded bg-slate-800 transition text-[10px] flex items-center gap-1 cursor-pointer disabled:opacity-40"
                  >
              ...
              <div className="flex items-center justify-between gap-2 bg-slate-900 px-2.5 py-1.5 rounded border border-indigo-500/20">
                <div className="flex items-center gap-1.5">
                  {(pin || activeUser.pin || '1234').split('').slice(0, 4).map((digit: string, idx: number) => (
                    <span
                      key={idx}
                      className="w-6 h-7 rounded bg-slate-950 border border-indigo-500/40 text-indigo-300 font-mono font-bold text-sm flex items-center justify-center"
                    >
                      {showPin ? digit : '•'}
                    </span>
                  ))}
            ...
            {/* Fecha de Registro */}
            <div className="flex items-center gap-2 p-2 bg-slate-950/40 rounded border border-slate-800/80">
              <Calendar className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <span className="text-[10px] text-slate-400">Registrado el:</span>
              <span className="text-[11px] font-medium text-slate-200">{formatDate(activeUser.createdAt)}</span>
            </div>
          </div>

          {/* Formulario de Actualizar Correo */}
          <form onSubmit={handleUpdate} className="pt-2 border-t border-slate-800 space-y-2">
            <div>
              <label className="block text-[10px] font-semibold text-slate-400 mb-1">Correo Electrónico</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-1.5 bg-slate-950 border border-slate-800 rounded-md text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 text-xs"
            ...
            <button
              ...
              className="w-full py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-md transition-all disabled:opacity-50 cursor-pointer shadow-xs"
            >
