...
export const MyAccount: React.FC<MyAccountProps> = ({ isOpen, onClose, user, onUpdateUser, onLogout }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-zinc-950/80 backdrop-blur-md flex items-start justify-center p-4 pt-14 sm:pt-20 overflow-y-auto">
      <div className="w-full max-w-sm bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200 rounded-xl shadow-2xl overflow-hidden text-xs shrink-0">
        {/* Header Compacto */}
        <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-950/80">
        ...
        <div className="p-3.5 space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded-lg border border-zinc-200 dark:border-zinc-800">
              <div className="flex items-center gap-2">
                <User className="w-4 h-4 text-indigo-400 shrink-0" />
                <div>
                  <p className="text-[9px] uppercase font-bold text-zinc-500 tracking-wider">Usuario</p>
                  <p className="text-xs font-semibold text-zinc-900 dark:text-white">{activeUser.name}</p>
                </div>
              </div>
            ...
            {/* PIN de Acceso */}
            <div className="p-2.5 bg-zinc-950/80 rounded-lg border border-indigo-500/30">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-0.5 px-1.5 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:text-white rounded bg-zinc-800 transition text-[10px] flex items-center gap-1 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    disabled={!pin}
                    className="p-0.5 px-1.5 text-zinc-600 dark:text-zinc-400 hover:text-indigo-300 rounded bg-zinc-800 transition text-[10px] flex items-center gap-1 cursor-pointer disabled:opacity-40"
                  >
              ...
              <div className="flex items-center justify-between gap-2 bg-zinc-900 px-2.5 py-1.5 rounded border border-indigo-500/20">
                <div className="flex items-center gap-1.5">
                  {(pin || activeUser.pin || '1234').split('').slice(0, 4).map((digit: string, idx: number) => (
                    <span
                      key={idx}
                      className="w-6 h-7 rounded bg-zinc-950 border border-indigo-500/40 text-indigo-300 font-mono font-bold text-sm flex items-center justify-center"
                    >
                      {showPin ? digit : '•'}
                    </span>
                  ))}
            ...
            {/* Fecha de Registro */}
            <div className="flex items-center gap-2 p-2 bg-zinc-950/40 rounded border border-zinc-200 dark:border-zinc-800/80">
              ...
              <span className="text-[11px] font-medium text-zinc-200">{formatDate(activeUser.createdAt)}</span>
            </div>
          </div>
          ...
          <form onSubmit={handleUpdate} className="pt-2 border-t border-zinc-200 dark:border-zinc-800 space-y-2">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-1.5 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200 rounded-md text-zinc-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 text-xs"
            ...
            <button
              ...
              className="w-full py-1.5 bg-indigo-600 hover:bg-indigo-500 text-zinc-900 dark:text-white text-xs font-semibold rounded-md transition-all disabled:opacity-50 cursor-pointer shadow-xs"
            >
