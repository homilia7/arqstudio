...
export const MyAccount: React.FC<MyAccountProps> = ({ isOpen, onClose, user, onUpdateUser, onLogout }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-zinc-950/80 backdrop-blur-md flex items-start justify-center p-4 pt-14 sm:pt-20 overflow-y-auto">
      <div className="w-full max-w-sm bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden text-xs shrink-0">
        {/* Header Compacto */}
        ...
        <div className="p-3.5 space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded-lg border border-zinc-800">
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-0.5 px-1.5 text-zinc-400 hover:text-white rounded bg-zinc-800 transition text-[10px] flex items-center gap-1 cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    disabled={!pin}
                    className="p-0.5 px-1.5 text-zinc-400 hover:text-indigo-300 rounded bg-zinc-800 transition text-[10px] flex items-center gap-1 cursor-pointer disabled:opacity-40"
                  >
            ...
            {/* Fecha de Registro */}
            <div className="flex items-center gap-2 p-2 bg-zinc-950/40 rounded border border-zinc-800/80">
              <Calendar className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
              <span className="text-[10px] text-zinc-400">Registrado el:</span>
              <span className="text-[11px] font-medium text-zinc-200">{formatDate(activeUser.createdAt)}</span>
            </div>
          </div>

          {/* Formulario de Actualizar Correo */}
          <form onSubmit={handleUpdate} className="pt-2 border-t border-zinc-800 space-y-2">
            <div>
              <label className="block text-[10px] font-semibold text-zinc-400 mb-1">Correo Electrónico</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-1.5 bg-zinc-950 border border-zinc-800 rounded-md text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 text-xs"
