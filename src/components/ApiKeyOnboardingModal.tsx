...
- Notificación obligatoria: POST ${baseUrl}/agent/notify-user con estado de progreso en vivo.`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-[#0e1117] border border-zinc-800 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden text-zinc-200">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-white tracking-tight flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-amber-950/50 text-amber-300 border border-amber-800/50">
                  Agentes Autónomos
                </span>
              </h2>
              <p className="text-[11px] text-zinc-400">
                Credenciales para conectar IAs externas (Antigravity, Codex, Hermes, Claude) al proyecto: <strong className="text-zinc-200">{projectName}</strong>
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 sm:p-5 space-y-4 overflow-y-auto text-xs text-zinc-300">
          ...
          {/* Key Box */}
          <div className="p-3.5 rounded-lg bg-[#12151b] border border-zinc-800 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Tu Clave API de Proyecto</span>
              <button
            ...
            <div className="p-2.5 rounded bg-[#090b0e] border border-zinc-800 font-mono text-xs text-amber-300 select-all break-all">
              {apiKey}
            </div>
            <div className="flex items-center justify-between text-[11px] text-zinc-500 font-mono">
              <span>Endpoint Base: <code className="text-zinc-400">{baseUrl}</code></span>
              <span>Project ID: <code className="text-zinc-400">{projectId}</code></span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-emerald-950/60 hover:bg-emerald-900/80 border border-emerald-800/80 text-emerald-300 text-xs transition-colors cursor-pointer font-medium"
              >
                {copiedPrompt ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{copiedPrompt ? "¡Prompt Copiado!" : "Copiar Prompt Completo"}</span>
              </button>
            </div>
            <pre className="p-3 rounded-lg bg-[#090b0e] border border-zinc-800 text-[11px] font-mono text-zinc-300 whitespace-pre-wrap leading-relaxed max-h-48 overflow-y-auto select-all">
              {systemPrompt}
            </pre>
          </div>

          {/* Steps */}
          <div className="p-3 rounded bg-zinc-900/50 border border-zinc-800 space-y-2 text-[11px]">
            <span className="font-bold text-zinc-200">Protocolo de Endpoints REST:</span>
            <div className="space-y-1 font-mono text-zinc-400">
              <div><span className="text-emerald-400">GET</span> /api/agent/welcome <span className="text-zinc-500">- Handshake y bienvenida</span></div>
              <div><span className="text-emerald-400">GET</span> /api/agent/next-task?projectId={projectId} <span className="text-zinc-500">- Tarea asignada</span></div>
              <div><span className="text-blue-400">POST</span> /api/projects/{projectId}/plan/batch <span className="text-zinc-500">- Cargar plan JSON</span></div>
              <div><span className="text-amber-400">POST</span> /api/agent/notify-user <span className="text-zinc-500">- Notificación en pantalla</span></div>
            </div>
          </div>
