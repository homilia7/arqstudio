...
export const HistoryDrawer: React.FC<HistoryDrawerProps> = ({
  ...
  const getAuthorBadge = (author: ChangeLogEntry["author"]) => {
    switch (author) {
      ...
      default:
        return (
          <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded text-[10px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
            <span>Sistema</span>
          </span>
        );
    }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex justify-end">
      <div className="bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 w-full max-w-xl h-full shadow-2xl flex flex-col transition-colors">
        {/* Header */}
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/80 dark:bg-slate-950/70">
        ...
        {/* Toolbar */}
        <div className="p-2.5 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="flex items-center space-x-1">
            <span className="text-slate-400 dark:text-slate-500 text-[11px] mr-1">Filtrar:</span>
            <button
          ...
          <div className="flex items-center space-x-1.5">
            <button
              ...
              className="inline-flex items-center space-x-1 px-2.5 py-1 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded border border-slate-200 dark:border-slate-700 text-[11px] font-medium transition-colors shadow-2xs cursor-pointer"
            >
            ...
            <button
              ...
              className="p-1 text-slate-400 dark:text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded transition-colors cursor-pointer disabled:opacity-40"
            >
          ...
          {filteredHistory.length === 0 ? (
            ...
            filteredHistory.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 space-y-2 hover:border-slate-300 dark:hover:border-slate-700 transition-colors shadow-2xs"
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        ...
                        className="text-white bg-rose-600 hover:bg-rose-700 active:bg-rose-800 p-1 rounded transition-all cursor-pointer font-black text-xs shadow border border-rose-500 hover:scale-105"
                      >
                  ...
                  <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed bg-slate-50 dark:bg-slate-950/60 p-2.5 rounded border border-slate-200/80 dark:border-slate-800/80">
                    {item.details}
                  </p>
                  ...
                  {item.workUrl && (
                    <div className="flex items-center space-x-1.5 text-[11px] text-slate-500 dark:text-slate-400">
                      ...
                      <span className="text-slate-400 dark:text-slate-500">URL asociada:</span>
                      <a
                        href={
